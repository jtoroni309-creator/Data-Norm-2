...
async def reset_password():
    ...
    async with engine.begin() as conn:
        ...
        # Update the user password - use password_hash column (not hashed_password)
        result = await conn.execute(
            text("UPDATE atlas.users SET password_hash = :pwd WHERE email = :email RETURNING id, email"),
